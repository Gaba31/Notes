...
-- Group By internal working
-- What group by does , it picks up column that you have selected 
-- for eg . dept and year
-- Now it forms a key and value in a seperate table
-- for eg key -> dept+year = IT2021
-- and value would be total employee in that year
-- If it read again that year and dept in appends the value in that only
-- That's how group by internally works.
```