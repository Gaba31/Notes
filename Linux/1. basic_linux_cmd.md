...
```bash
 ...
 touch 'a b'                       #Within quote it will treat as single file
```
...
```bash
ls >> a.txt
# With this it will override standard output in  a.txt.
...
```bash
echo $((cd Downloads/ ; ls)) #First it will go to downloads then run ls but still we are in the same directory

echo $((cd Downloads/ || ls)) # Behave as OR

echo $((cd Downloads/ && ls)) # Behave as &&
```
<hr>
